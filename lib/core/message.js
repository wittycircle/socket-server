...
const getAllDiscussions = exports.getAllDiscussions = (id) => {
    let last = db('old_messages as n').select('n.message', 'n.from_user_id').where('n.to_user_id', id).orderByRaw('n.creation_date DESC').groupBy('n.from_user_id').as('nik')
    dbms.db.sql
        .from('old_messages as m')
        .distinct(['m.from_user_id as id', db.raw(`CONCAT(p.first_name, ' ', p.last_name) AS full_name`),
            'p.profile_picture as picture', db.raw('MAX(m.creation_date) as date'), 'm.m_read',
    ...
    return Promise.all([
        dbms.db.sql.from('old_messages as m')
            .distinct([
                'm.from_user_id as id',
                'm.id as reference',
                dbms.db.sql.raw(`CONCAT(p.first_name, ' ', p.last_name) AS full_name`),
                'p.username',
                'p.profile_picture as picture',
                dbms.db.sql.raw(`SUBSTRING_INDEX(GROUP_CONCAT(m.message order by m.creation_date desc), ',', 1) as last_message`),
        ...
        dbms.db.sql.from('old_messages as m')
            ...
            .join('users as u', 'u.id', 'm.to_user_id')
            .join('profiles as p', 'p.id', 'u.profile_id')
            .where('m.from_user_id', id)
            .groupBy('m.from_user_id')
            .orderByRaw('date DESC')
    ])
        .then(results => {
        ...
        .then(discussions => _.sortBy(discussions, 'date')
            .map(discussion => {
            return {
                id: discussion.id,
                from: {
