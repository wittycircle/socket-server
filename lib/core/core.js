...
class Core {
    ...
    constructor(opts) {
        ...
        this._defaultListener = (bind) => Rx.Observer.create(
            x => {
                if (bind == 'rest:update:user_activity') return; //todo remove this
                console.log(`DefaultListener: ${bind}: ${x}`)
            },
        ...
        if (process.env.NODE_ENV === 'development') {
            redis.monitor(function (err, monitor) {
                monitor.on('monitor', function (time, args, source, database) {
                    if (Array.isArray(args) && (args.length > 2 && args[1] == 'notifications:recent'))
                        console.log('MONITOR', time, args, source, database)
                });
            });
        }
    ...
    _start() {
        ...
        this._channels.forEach(channel => {
            ...
            this._subjects[channel]
                .subscribe(this._defaultListener(channel))
        });
