                    ...
                    .subscribe(token => {
                            session.getUser(token)
                                ...
                                .then(user => {
                                    ...
                                    cache.db.redis.lrange(`notifications:${user.id}`, 0, -1)
                                        .then(notifications => {
                                            console.log(`User:${user.id} - ${notifications.length} notifications in storage`);
                                            client.socket.emit('server::notification', notifications.map(JSON.parse));
                                        });
