    ...
    client.privateStreams('discussions')
        .subscribe((data) => {
            message.getAllDiscussions(client.user.id)
                .then(results => {
                    let unread = 0;
                    results.forEach(r => r.read ? unread++ : {});
                    console.log(`User:${client.user.id} - ${results.length} discussions - ${unread} unreads`);
    ...
    client.privateStreams('notifications::fetch')
        .subscribe(() => {
            ...
            cache.db.redis
                ...
                .get(`${client.user.id}:last`)
                .exec()
                .then(notifications => {
                    const hydrate_notif = (raw) => {
                        let n = JSON.parse(raw);
                        n.read = n.when > +notifications[1][1];
                        return n;
                    };
