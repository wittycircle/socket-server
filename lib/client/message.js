    ...
    return redis.smembers(`users:online`).then(onlines => {
        return yy.then(r => _.uniqBy(r, e => e.from))
            .then(discussions => discussions.map((discussion, i) => {
                let online = Array.isArray(onlines[1]) && onlines[1].includes(discussion.uid);
                return {
                    id: discussion.uid,
                    from: {
                        id: discussion.uid,
...
const setAllRead = exports.setAllRead = (from, to) => {
    let mt = db('old_messages').distinct('id').where('from_user_id', from).andWhere('to_user_id', to).as('mt'),
    ...
    return db('old_messages').update('m_read', 1).whereIn('id', discussion_id)
};
