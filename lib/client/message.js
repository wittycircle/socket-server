...
exports.getAllDiscussions = id => {
  ...
  let testmerde = db('room_status as rs')
    .select('user_id', 'creation_date', 'room_id', 'rs.read', 'mail_sent', 'id')
    .where('user_id', id)
    .orderByRaw('creation_date DESC');

  let statusThing = db
    .select('*')
    .from(db.raw(`(${testmerde}) as nini`))
    .groupBy('room_id')
    .as('rs');
  ...
  let query = db('messages as m')
    ...
    .join(statusThing, 'rs.room_id', 'm.room_id')
    .distinct([
      'r.id',
      'r.name',
      last_message,
