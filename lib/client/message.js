...
exports.getAllDiscussions = id => {
  ...
  // select * from room_status as rs
  // join
  // (select max(id) as id, user_id, room_id, mail_sent, `read`,
  //  max(creation_date) as dateOrder
  //  from room_status
  //   group by room_id order by room_id)
  //   sr on sr.id = rs.id order by creation_date;

  let roomStatus = db('room_status as rs')
    .select('*')
    .join(
      db('room_status')
        .select(
          db.raw('max(id) as maxId'),
          db.raw('max(creation_date) as dateOrder')
        )
        .groupBy('room_id')
        .orderBy('room_id')
        .as('sr'),
  ...
  let query = db('messages as m')
    ...
    .distinct([
      ...
      db.raw(`if(rs.user_id <> ${id}, 1, rs.read) as 'read'`),
      'rs.mail_sent'
    ])
    .where('rm.user_id', id)
    .groupBy('m.room_id')
