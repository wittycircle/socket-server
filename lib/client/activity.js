...
const HYDRATE = {
    ...
    'user_follow': (data) => {
        return Promise.all([HELPER.USER_META(data.from), HELPER.USER_META_FROM_PROFILE(data.id)])
            .then(results => {
                return {
                    when: data.when,
                    type: 'user_follow',
...
module.exports = (events) => {
    ...
    EVENTS.forEach(event => {
        let OBS = when(event)
            ...
            .filter(event => event.from.full_name.length && event.to.what.length); //remove untracked items
