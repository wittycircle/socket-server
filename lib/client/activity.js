...
const HYDRATE = {
  project_up: data => {
    return Promise.all([
      HELPER.USER_META(data.from),
      HELPER.PROJECT_META(data.id)
    ]).then(results => {
      return {
        when: data.when,
        type: 'project_up',
  ...
  opening_creation: data => {
    return HELPER.PROJECT_META(data.from).then(results => {
      return {
        when: data.when,
...
module.exports = events => {
  ...
  const when = event => events.from(event),
    storeEvent = type => data => {
      limit(data)(ONE_HOUR, `activity:${type}:${data.from}:${data.id}`, () =>
        HYDRATE[type](data).then(result => {
          console.log(
            `New activity: ${result.type} from ${result.from.full_name} route=${result.route} params=${result.params || null}`
          );
          SUB.onNext(true);
          if (result.from.full_name && result.from.picture)
            redis
              .multi()
  ...
  EVENTS.forEach(event => {
    let OBS = when(event)
      .filter(
        event => (Object(event).hasOwnProperty('value') ? event.value == 1 : true)
      )
