...
exports.init = () => {
    Promise.all([
        ...
        db(`${TABLES.USERS} as u`)
            ...
            .then(query => {
                redis
                    .multi(query.users).zadd('autocomplete:username', query.username)
                    .exec()
            })
            .catch(console.error),
        /**
         ...
         */
        db.raw(`SELECT p.id, p.title, p.project_visibility, p.user_id, p_f.upvoters FROM projects AS p
        ...
        .then(results => console.log('Cache bootstrap successful'))
};
