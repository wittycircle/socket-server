...
const needsBootstrap = (duration) =>
  redis.ttl('cache:bootstrap').then(value => {
    if (value == -2)
      return redis
        .multi()
        .set('cache:bootstrap', Date.now())
...
exports.init = () => needsBootstrap(60)
  .then(state => {
    if (state) return init()
      .then(() => console.log('Cache bootstrap successful'));
    else {
      console.log('Cache bootstrap prevented');
      return true;
    }
  });