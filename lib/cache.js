...
exports.init = () => {
    return Promise.all([
        ...
        db(`${TABLES.USERS} as u`)
            .select(['u.id', 'u.email', 'u.username', 'p.first_name', 'p.last_name', 'profile_id', 'profile_picture'])
            .orderBy('u.id')
         ...
         */
        db.raw(`SELECT p.id, p.title, p.public_id, p.project_visibility, p.picture, p.user_id, p_f.upvoters FROM projects AS p
