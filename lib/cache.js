...
exports.init = () => {
    ...
    db(TABLES.USERS).select(['id', 'username']).orderBy('id')
        .then(users => users.map(user => [user.id, user.username]))
        .then(users => redis.zadd('autocomplete:username', _.flattenDeep(users)))
        .catch(console.error);
    // db.raw(`
